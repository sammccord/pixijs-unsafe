...
export class GlobalUniformSystem implements System {
    ...
    public bind({
        ...
        if ((this._renderer as unknown as WebGPURenderer).renderPipes.uniformBatch) {
            bindGroup = (this._renderer as unknown as WebGPURenderer).renderPipes.uniformBatch.getUniformBindGroup(uniformGroup, false);
        }
        else {
